...
CategorySchema.methods.getPriceRanges = function (query, cb) {
  var _this = this;
  _this.getMinMax(query, function (err, minMax) {
    if (err) return cb(err);
    var splitNumber = 6;
...
CategorySchema.methods.getMinMax = function (query, cb) {
  var _this = this;
  async.parallel([
    function (callback) {
      _this.getHighestOrLowestPrice(query, 'price', callback);
    },
    function (callback) {
      _this.getHighestOrLowestPrice(query, '-price', callback);
    }
...
CategorySchema.methods.getHighestOrLowestPrice = function (query, sort, cb) {
