...
router.post('/', function (req, res, next) {
  ...
  product.save(function (err, product) {
    ...
    Product.populate(product, '_user', function (err, product) {
      if (image) {
        writeImage('storage/images/products/' + product._id + '.jpg', image, function () {
          res.json(product);
        });
      } else {
        res.json(product);
...
router.delete('/:id', function (req, res, next) {
  Product
    .remove({
      ...
      if (err) return next(err);
      fs.unlink('storage/images/products/' + req.params.id + '.jpg', function (err) {
        if (err) next(err);
      });
...
router.put('/:id', function (req, res, next) {
  ...
  Product
    .update({
      ...
      if (err) return next(err);
      if (image) {
        writeImage('storage/images/products/' + req.params.id + '.jpg', image, function () {
          res.json(product);
        });
      } else {
        res.json(product);
